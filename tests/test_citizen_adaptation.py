...
def _signed_descriptor(private_key, artifact_hash):
    unsigned = {
        "version": "1.0.1",
...
def test_release_descriptor_and_artifact_verification(tmp_path):
    ...
    verifier = ReleaseVerifier(public, "test-key-1")
    descriptor = _signed_descriptor(private_key, digest)
...
def test_evolution_prepare_commit_and_rollback(tmp_path):
    ...
    digest = hashlib.sha256(artifact.read_bytes()).hexdigest()
    private_key = Ed25519PrivateKey.generate()
    verifier = ReleaseVerifier(private_key.public_key().public_bytes_raw(), "test-key-2")
    descriptor = _signed_descriptor(private_key, digest)
    descriptor["signing_key_id"] = "test-key-2"
    descriptor["signature"] = base64.b64encode(private_key.sign(verifier.canonical_payload(descriptor))).decode("ascii")
...
def test_evolution_rejects_bad_artifact(tmp_path):
    ...
    artifact.write_bytes(b"v2")
    private_key = Ed25519PrivateKey.generate()
    verifier = ReleaseVerifier(private_key.public_key().public_bytes_raw(), "test-key-3")
    descriptor = _signed_descriptor(private_key, "0" * 64)
    descriptor["signing_key_id"] = "test-key-3"
    descriptor["signature"] = base64.b64encode(private_key.sign(verifier.canonical_payload(descriptor))).decode("ascii")
