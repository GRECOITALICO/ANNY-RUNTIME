...
1. **Durable Source of Truth**: Conversation memory and transient process state are ephemeral interfaces. The canonical operational state resides in `GRECOITALICO/ANNY-OPERATIONAL`.
...
3. The bootstrap resolver re-reads `GRECOITALICO/ANNY-OPERATIONAL` from GitHub/local storage.
